...
def decode_jwt(token: str, secret: str) -> dict:
    ...
    signature = hmac.new(secret.encode(), f"{header_b64}.{payload_b64}".encode(), hashlib.sha256).digest()
    expected_signature_b64 = base64.urlsafe_b64encode(signature).decode().replace("=", "")
...
async def monitor_requests(request: Request, call_next):
    ...
    API_REQUESTS_TOTAL.labels(method=request.method, endpoint=endpoint, status=response.status_code).inc()
...
@app.get("/scenarios", tags=["Scenarios"], dependencies=[Depends(RoleChecker(["viewer", "operator", "admin"]))])
def list_scenarios():
    return {
        "scenarios": [
            {"name": name, "sensor": s["sensor"], "event": s["event"], "severity": s["severity"]}
            for name, s in SCENARIOS.items()
        ]
    }

@app.get("/scenario/{name}", tags=["Scenarios"], dependencies=[Depends(RoleChecker(["viewer", "operator", "admin"]))])
...
@app.post("/diagnose", response_model=DiagnosisResponse, tags=["Diagnosis"], dependencies=[Depends(RoleChecker(["operator", "admin"]))])
def diagnose_event(event: EventInput):
    return _run_diagnosis(event.model_dump())

@app.post("/diagnose/scenario", response_model=DiagnosisResponse, tags=["Diagnosis"], dependencies=[Depends(RoleChecker(["operator", "admin"]))])
def diagnose_by_scenario(req: ScenarioRequest):
    if req.name not in SCENARIOS:
        raise HTTPException(status_code=404, detail="Scenario not found")
    return _run_diagnosis(SCENARIOS[req.name])

@app.get("/results", tags=["History"], dependencies=[Depends(RoleChecker(["viewer", "operator", "admin"]))])
...
@app.delete("/results", tags=["History"], dependencies=[Depends(RoleChecker(["admin"]))])
def clear_results():
    _save_results([])
    return {"message": "Results cleared."}

@app.post("/remediate", tags=["Remediation"], dependencies=[Depends(RoleChecker(["operator", "admin"]))])
def remediate(req: RemediateRequest):
    ...
    if policy == ApprovalMode.AUTO:
        ...
        REMEDIATIONS_TOTAL.labels(action=req.action, status=result["status"], policy="AUTO").inc()
...
@app.get("/approvals", tags=["Remediation"], dependencies=[Depends(RoleChecker(["operator", "admin"]))])
...
@app.post("/approvals/{request_id}/approve", tags=["Remediation"], dependencies=[Depends(RoleChecker(["operator", "admin"]))])
def approve_action(request_id: str, body: ApprovalActionRequest):
    ...
    REMEDIATIONS_TOTAL.labels(action=approval.action, status=result["status"], policy="MANUAL").inc()
...
@app.post("/approvals/{request_id}/reject", tags=["Remediation"], dependencies=[Depends(RoleChecker(["operator", "admin"]))])
def reject_action(request_id: str, body: ApprovalActionRequest):
    try:
        approval = _approval_manager.reject(request_id, rejected_by=body.resolved_by, reason=body.notes)
    except KeyError:
        raise HTTPException(status_code=404, detail="Request not found")
    except ValueError as e:
        raise HTTPException(status_code=409, detail=str(e))
...
@app.get("/audit", tags=["Governance"], dependencies=[Depends(RoleChecker(["viewer", "operator", "admin"]))])
...
@app.get("/incidents", tags=["Governance"], dependencies=[Depends(RoleChecker(["viewer", "operator", "admin"]))])
